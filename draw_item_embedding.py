...
def computeNearestNeighbor(weights):
    ...
    for i in xrange(len(weights)):
        if i+1 in item_dict:
            ...
            if 'recipe' in name:
                continue
            picture_file = 'result/items/' + name + '.png'
            temp =  '| <img src=%s style="height:30px; width:30px"></img>|' % picture_file
            for j in xrange(1, len(indices[i])):
                if indices[i][j]+1 in item_dict:
                    name = item_dict[indices[i][j]+1].lower().replace(' ','')
                    # ignore all recipe
                    if 'recipe' in name:
                        continue
                    picture_file = 'result/items/' + name + '.png'
...
def generateEmbeddingHTML(pos, height, width, embed_type, image_size=(50,50)):
    ...
    result_file = open('result/result_item_%s.html' % embed_type, 'w')
    ...
    for i in xrange(len(pos)):
        if (i+1) in item_dict:
            temp_name = item_dict[i+1]
            # ignore all recipe
            if 'recipe' in temp_name:
                continue
            picture_file = 'items/' + temp_name + '.png'
